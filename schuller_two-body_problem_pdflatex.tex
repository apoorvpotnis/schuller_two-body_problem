...
\usepackage{amsmath, amssymb, bm, mlmodern}
\let\mathbb\mathbf
...
% % \newcommand{\ltwo}{\mathrm{L\kern-0.5pt^2}}
% \newcommand{\position}{\mathrm{Q}}
% \newcommand{\momentum}{\mathrm{P}}
% \newcommand{\rthree}{\mathbb{R}^3}
% \newcommand{\rr}{\mathbb{R}}
% \newcommand{\cc}{\mathbb{C}}
% \newcommand{\nn}{\mathbb{N}_0}
% \newcommand{\dirac}{\mathrm{\delta}}
...
% \newcommand{\resolvent}{\mathrm{\rho}}

\newcommand{\ltwo}{\mathrm{L\kern-0.7pt^2}}
...
\newcommand{\resolvent}{\mathrm{\rho}}
\newcommand{\lap}{\Delta}
% \renewcommand{\mathbfit}[1]{\mathbfit{#1}}
\newcommand{\xone}{\mathbfit{x}^{(1)}}
\newcommand{\xtwo}{\mathbfit{x}^{(2)}}
\newcommand{\rvec}{\mathbfit{r}}
\newcommand{\yvec}{\mathbfit{y}}
\newcommand{\angsone}{\tilde{L}_1}
\newcommand{\angstwo}{\tilde{L}_2}
\newcommand{\angsthree}{\tilde{L}_3}
\newcommand{\lvecsquare}{\tilde{\mathbfit{L}}\kern-1pt^2}
...
\begin{document}
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\tkzDrawArc[R, color=black, line width=0.8pt, -{>[bend]}](O,1.1cm)(340,10)
			\tkzDrawArc[R, color=black, line width=0.8pt, -{>[bend]}](O,1.1cm)(160,190)
	...
	\]
	where \(\lap_{(i)}\) is the Laplacian operator which acts on the Hilbert space \(\ltworthree\) of the particle \(i\), and \(\mathbfit{x}^{(i)} \in \rthree\) denotes the position vector of the \(i^\text{th}\) particle. Assuming that this Hamiltonian is self-adjoint, we are interested in finding out its spectrum.
	...
	Actually, as we only discuss the orbital angular momentum operators, i.e.\ \(L_i\)'s in this lecture, we could have used a slightly different approach to justify the procedure using ladder operators. We know that each \(L_i\) has a pure point spectrum contained within the integers. By applying the spectral mapping theorem, we get that polynomials in \(L_i\), defined using spectral calculus, have the same spectrum as \(L_i\), i.e.\ if \(p\) is a polynomial, then
	\[
	\sigma(p(L_i)) = p(\sigma(L_i)).
	\]
	If we take \(p(x) = x^2\), then we get that \(\sigma(L_i^2) \in \zz\), as \(\sigma(L_i) \in \zz\). We have a further result that if two operators have a pure point spectrum and they commute, then there exists a common Schauder eigenbasis of both the operators. This justifies the assumption of considering simultaneous eigenvectors of \(\bm{L}^2\) and \(L_i\) in the procedure using ladder operators.
	...
	\[
	\psi\kern0.5pt_l^m (\tilde{\mathbfit{x}} (r, \theta, \phi)) = k \cdot Y^m_l (\theta, \phi) \cdot f(r),
	\]
	where
	\[
	...
	\]
	and $\tilde{\mathbfit{x}} = T^{-1}(\mathbfit{x})$. These $Y_l^m$'s are called as \textit{spherical harmonic functions}. Since the operators $\lvecsquare$ and $\tilde{L}_3$ act only on the angular components and not the radial components when expressed in spherical co-ordinates, we have that
	\begin{align*}
