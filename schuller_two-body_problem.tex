...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
    ...
    \begin{thm}[Spectral Mapping]
        Let \(A\) be a self-adjoint operator on a Hilbert space \(\hilbert\), and let \(f \colon \rr \rightarrow \cc\) be a continuous function. Then
        \[
            \spec((f(A)) = f(\spec(A)),
        \]
        if \(\abs{f(\lambda)} \rightarrow \infty\) as \(\abs{\lambda} \rightarrow \infty\).
    \end{thm}
    Here, \(f(A)\) is defined using the spectral calculus. By applying the spectral mapping theorem, we get that complex polynomials in \(L_i\) have the same spectrum as \(L_i\), i.e.\ if \(p\) is a complex polynomial, then
    \[
        \spec(p(L_i)) = p(\spec(L_i)).
    \]
    If we take \(p(x) = x^2\), then we get that \[\spec(L_i^2) \in \zz,\] as \(\spec(L_i) \in \zz\). We have a further result that if two self-adjoint operators on a complex Hilbert space have a pure point spectrum, and they commute, then there exists a common orthonormal eigenbasis of both the operators. This result is asked as problem~4.5 in the book of Teschl~\cite[p.~138]{Teschl}. This justifies the assumption of considering simultaneous eigenvectors of \(\symbfit{L}^2 \coloneq L_1^2 + L_1^2 + L_3^2\) and \(L_3\) in the usual procedure using ladder operators.
