...
% \lvdset{glyph={show=true}}

% code adapted from Marcel Kruger's answer https://tex.stackexchange.com/a/760117/128462
...
\begin{luacode}
    local kerns = {
        ["NewCMMath-Book"] = {
          ...
          ["u1D6F7"] = {bottomright = { { kern = 0 } } }, -- mathematical italic capital Phi
          ["u1D6F9"] = {bottomright = { { kern = 0 } } }, -- mathematical italic capital Psi
          --["u1D464"] = {bottomright = { { kern = -90 } } }, -- mathematical italic small a
          ["u1D460"] = {bottomright = { { kern = -20 } } }, -- mathematical italic small s
          ["u1D437"] = {bottomright = { { kern = 0 } } }, -- mathematical italic capital H
          ["L"] = {topright = { { kern = -50 } } },
          ["u1D43D"] = {bottomright = { { kern = 0 } } }, -- mathematical italic capital J
          ["u1D70E"] = {bottomright = { { kern = -80 } } }, -- mathematical italic small sigma
          --["u1D457.sts"] = {topleft = { { kern = -1000 } } }, -- mathematical italic small j subscript level 1
        },
        ["NewCMMath-Bold"] = {
...
\begin{filecontents}[overwrite]{schuller_two-body_problem.bib}
    ...
    @book{MorettiSpectral,
        ...
        publisher = {Springer Nature Switzerland \textsc{ag}},
    ...
    @book{MorettiFundamental,
        ...
        subtitle = {Spectral Theory, Foundational Issues, Symmetries, Algebraic Formulation},
        year = {2019}
    }
    ...
    @book{Teschl,
        ...
        subtitle = {With Applications to Schr\"odinger Operators},
        year = {2014},
        series = {Graduate Studies in Mathematics},
        volume = 157,
        addendum = {The book is freely available in \texttt{.pdf} format on the author's website at \url{https://www.mat.univie.ac.at/~gerald/ftp/book-schroe/schroe.pdf}.}
    }
...
\renewcommand{\mitsansXi}{\hat{\Xi}}
\newcommand{\cpi}{\ensuremath{\kern0.3pt\text{п}}}
...
% 			\draw[black, thick, ->, xshift=0.8cm, yshift=-0.8cm] (-30:0.8cm) arc (-30:-10:0.8cm);
    ...
    \begin{defn}[Strongly continuous unitary representation]
        ...
        \end{align*}
        be a unitary representation on \(\hilbert\). Let \(\domain \subseteq \hilbert\) be dense in \(\hilbert\). We say that \(U\) is strongly continuous if for all \(\psi \in \domain\),
        \[
    ...
    \begin{defn}[Self-adjoint generator associated with an element of a Lie algebra]
        ...
        \begin{align*}
            U^{\mitsansXi} &\colon \rr \rightarrow \exp(s\mitsansXi),\\
            U^{\mitsansXi} &\colon s \mapsto G,
        \end{align*}
        be the one-parameter Lie subgroup of \(G\) generated by \(\mitsansXi\). Then, the self-adjoint generator associated with \(\mitsansXi\), \(\Xi \colon \domain_{\Xi} \rightarrow \hilbert\), is the generator of the strongly continuous one-parameter unitary group \(G_{\mitsansXi} \coloneq \operatorname{ran}(W)\), defined by
        \begin{align*}
    ...
    \begin{thm}[Nelson]
        Consider a real \(n\)-dimensional Lie algebra \(V\) of operators \(-\i S_1\), \dots, \(-\i S_n\), where each \(S_i\) with \(i \in \{1, \dots, n\}\) is essentially self-adjoint on the Hilbert space \(\hilbert\), defined on a common invariant and dense subspace \(\domain \subset \hilbert\), with the usual commutator as the Lie bracket.
        Let \(-\i S_1, \dots, -\i S_n \in V\) be a basis of \(V\) and define Nelson's operator with domain \(\domain\) as
        \[
        ...
        \(U\) is uniquely determined by the fact that the closures \(\bar{S}\), for every \(-\i S \in V\), are the self-adjoint generators of the representations of the one-parameter subgroups of \(G_V\).
    \end{thm}
    ...
    \(\sutwo\) is the Lie algebra of both \(\SUtwo\) and \(\SOthree\). There is a two-to-one correspondence from \(\SUtwo\) to \(\SOthree\). \(\SOthree\) is not simply connected. The unique simply-connected Lie group associated to \(\sutwo\) is \(\SUtwo\). Thus, Nelson's theorem guarantees the existence of a unique strongly continuous unitary representation \(\tilde{U} \colon \SUtwo \rightarrow B(\hilbert)\).\footnote{Note that the closure of \(J_i\), \(\bar{J}_i\), is the self-adjoint generator of the one-parameter subgroup \( \tilde{U}( (\SUtwo)_{-\i J_i}) \).}
    ...
    \[
        \bar{J_k} \colon \domain_{J_k} \rightarrow \ltworthree
    \] 
    is a self-adjoint operator, with the Stone domain
    \[
        \domain_{\bar{J_k}} = \bigg\{ \psi \in \ltworthree \biggm| \lim_{\epsilon \rightarrow 0} \frac{\i}{\epsilon} (U_k(\epsilon)\psi - \psi) \text{ exists.}\bigg\},
    \]
    where \(\epsilon \in \rr\), and
    \[
        \exp(-\i \bar{J_k} \alpha) = U_k(\alpha).
    \]
    \(J_k\) is an essentially self-adjoint operator with \(\schwartzrthree\) as the core. Each \(\ltworthree_n\) is invariant under the action of any unitary one-parameter group \(U_k\), by the Peter--Weyl theorem. Each \(\ltworthree_n\) is finite-dimensional, and every vector in \(\psi \in \ltworthree_n\) is smooth for \(U_k\), i.e. the mapping
    \begin{align*}
        s_k &\colon \rr \rightarrow \ltworthree_n,\\
        s_k &\colon \alpha \mapsto (U_k(\alpha))\psi,
    \end{align*}
    is differentiable in the norm topology on \(\ltworthree_n\) for each \(k\). This is due to the fact that \(U_k(\alpha) = \exp(-\i \alpha \bar{J_k})\) is analytic in \(\alpha\). Thus, each \(\ltworthree_n \subset \domain_{\bar{J_k}}\).\footnote{Note that even though each subset, \(\ltworthree_n\), belongs to the domain, \(\domain_{\bar{J_k}}\), their direct sum \(\ltworthree\) does not. The direct sum in the decomposition of \(\ltworthree\) is a Hilbert direct sum, and not an algebraic direct sum. An element of the algebraic direct sum consists of a \textit{finite} linear combination of vectors, with atmost one vector from each subspace. The algebraic direct sum is dense in the Hilbert direct sum.}

    Thus, the usual spectral analysis for angular momentum operators using ladder operators holds, since all \(\ltworthree_n\) are finite-dimensional. Thus, using the theorems of Nelson and Peter--Weyl, and the subsequent ladder operator analysis, we get that $\bar{J_i}$'s have a pure point spectrum, and that the common eigenvectors of $\symbfit{J}^2$ and $J_3$ come as families $\psi_{j(j+1), m}$, where $m = -j, -j+1, \dots, j-1, +j$. The eigenvalue $j(j+1)$ is associated to $\symbfit{J}^2$ and $m$ is associated to $J_3$ with $j \in \symbb{N}_0/2$.
    ...
    \[
        A \coloneq [0, +\infty) \times [0,\cpi] \times [0,2\cpi).
    \]
    Let $T$ be defined as
    \begin{align*}
    ...
    \(\schwartzrthree\), \(\schwartz(\tilde{\rr}^3)\), \(\schwartz(A)\), and \(\schwartz(\tilde{A})\) are all isomorphic to each other. Thus, the eigenvalues and eigenvectors of \(\symbfit{L}^2\) and \(L_i\)'s, and \(\lvecsquare\) and \(\tilde{L}_i\)'s are the same, respectively. The self-adjointness is unaffected by these coordinate changes as well. Thus, we know that the common eigenvectors of $\lvecsquare$ and $\tilde{L}_3$ come as families $\psi_l^m \in \schwartz(\tilde{A})$, where $m = -l, -l+1, \dots, l-1, l$ and $l \in \nn$. We have
    \begin{align*}
        \lvecsquare \psi_l^m &= l(l+1)\psi_l^m,\\
        \tilde{L}_3 \psi_l^m &= m \psi_l^m.
    \end{align*}
    Note that $l$ can take only integer values, not half-integer values, as we are dealing with the orbital angular momentum operator. In spherical coordinates, it turns out that the solutions $\psi_l^m$ are given by
    \[
        \psi_l^m (\tilde{\symbfit{x}} (r, \theta, \phi)) = k \cdot Y^m_l (\theta, \phi) \cdot f(r),
    \]
    where
    \[
        Y^m_l (\theta, \phi) \coloneq \frac{(-1)^m}{2^l l!}\left(\frac{(2l+1) (l-m)!}{4\cpi (l+m)!}\right)^{\frac{1}{2}}\!\cdot\e^{\i m\phi}\cdot\!(\sin{\theta})^m \left(\frac{\partial}{\partial \cos{\theta}}\right)^{l+m}\!\!\!(\cos^2{\theta} -1)^l
    \]
    ...
    \[
        \langle \sphe , Y_{l'}^{m'} \rangle = \delta_{l, l'}\cdot\delta_{m, m'}.
    \]
    ...
    The solutions of a differential equation such as \eqref{eigen} depend on the boundary conditions. We have already assumed that \(\hrel\) is self-adjoint on a suitable domain. For our purposes, it is sufficient to restrict our class of potentials which do not increase more rapidly than the Coulomb potential at the origin. More precisely, we require that the the following condition is satisfied.
    \[
        \lim_{r \rightarrow 0} rV(r) = c,
    \]
    where \(c \in \rr\) is a constant. To obtain normalized solutions, the solutions also must not increase too rapidly at the origin. More precisely, we need the following condition.
    \[
