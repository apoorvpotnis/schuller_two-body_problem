...
\newcommand{\sutwo}{\symfrak{su}(2)}
\newcommand{\sltwoc}{\symfrak{sl}(2;\cc)}
...
\begin{document}
    ...
    The source code, updates and corrections to this document can be found on this GitHub repository: \url{https://github.com/apoorvpotnis/schuller_two-body_problem}. The source code, along with the \texttt{.bib} file is embedded in this PDF. Comments and corrections can be mailed at \href{mailto:apoorvpotnis@gmail.com}{\texttt{apoorvpotnis@gmail.com}}.
...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
    ...
    \begin{thm}[Nelson]
        ...
        \[
            \Delta_{\symrm{N}} \coloneq \sum_{k=1}^{n} S_k^2.
        \]
    ...
    \begin{thm}[Peter--Weyl]
        ...
        \begin{enumerate}
            ...
            \item Every map \(\restr{U}{\hilbert_k} \colon \hilbert_k \rightarrow \hilbert_k\) is an irreducible representation of \(G\).
        \end{enumerate}
    \end{thm}

    \(\operatorname{SU(2)}\) is a compact group. And thus, strongly-continuous unitary representations of \(\operatorname{SU}(2)\) on \(\ltworthree\) decompose \(\ltworthree\) into finite dimensional subspaces. \(\ltworthree_n\) denotes such a subspace. Thus,
    \[
    ...
    Actually, as we only discuss the orbital angular momentum operators, i.e.\ the \(L_i\)'s in this lecture, we could have used a slightly different approach to justify the procedure using ladder operators. We know that each \(L_i\) has a pure point spectrum contained within the integers, i.e.\ for all \(i\),
    \[
        \spec(L_i) \subseteq \zz.
    \]
    We state a version of the spectral mapping theorem, referring to theorem~3.17 of the book of Teschl~\cite[p.~118]{Teschl}.
    ...
    \]
    If we take \(p(x) = x^2\), then we get that \[\spec(L_i^2) \subseteq \zz,\] as \(\spec(L_i) \subseteq \zz\). We now need the following result
    \begin{thm}
        Tf two commuting self-adjoint operators on a complex Hilbert space have a pure point spectrum, then there exists a common orthonormal eigenbasis of both the operators.
    \end{thm}
    This result has been asked as problem~4.5 in the book of Teschl~\cite[p.~138]{Teschl}. This justifies the assumption of considering simultaneous eigenvectors of \(\symbfit{L}^2 \coloneq L_1^2 + L_1^2 + L_3^2\) and \(L_3\) in the usual procedure using ladder operators.
    ...
    We know that a common orthonormal eigenbasis of $\symbfit{L}\kern-1pt^2$ and $L_i$ exists. Depending on \(H_\text{rel}\), if a common orthonormal eigenbasis of \(\symbfit{L}\kern-1pt^2\), \(L_i\), and \(H_\text{rel}\) exists, then, as all three operators commute with each other pairwise, and since the eigenvectors and eigenvalues of an operator are not affected by co-ordinate changes up to isomorphism, \(\schwartzrthree\), \(\schwartz(\tilde{\rthree})\), \(\schwartz(A)\), and \(\schwartz(\tilde{A})\) are all isomorphic to each other. The self-adjointness is unaffected by the co-ordinate changes as well.
