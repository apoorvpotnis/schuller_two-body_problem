...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
	...
	\begin{thm}[Peter--Weyl]
		...
		\begin{enumerate}
			...
			\item Every map \(U\restriction_{\hilbert_k} \colon \hilbert_k \rightarrow \hilbert_k\) is an irreducible representation of \(G\).
		\end{enumerate}
	\end{thm}
	...
	\]
	\(\schwartzrthree_n\) is invariant under the action of \(J_i\)'s for all \(n\). This was the procedure Prof. Schuller briefly mentioned. Now, since \(J_i\restriction_{\schwartzrthree_n} \colon \schwartzrthree_n \rightarrow \schwartzrthree_n\) acts on finite dimensional spaces, it spectrum consists of only eigenvalues, i.e.\ each \(J_i\restriction_{\schwartzrthree_n}\) has a pure-point spectrum. The spectrum of \(J_i\)'s just the union of these individual eigenvalues, i.e.\
	\[
		\sigma(J_i) = \bigcup_{n \in \nn} \sigma(J_i\restriction_{\schwartzrthree_n}).
	\]
