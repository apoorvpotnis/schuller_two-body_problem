...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
    ...
    \]
    Let us restrict the Hilbert space \(\ltworthree\) to the Hilbert \(\bhilbert\) space spanned by the common eigenvectors of \(\lvecsquare\), \(\tilde{L}_i\), and \(\hrel\).\footnote{We choose ``b'' as a subscript in the label \(\bhilbert\) as this space shall later turn out to the space of \textit{bound states}.} Thus,
    \[
        \bhilbert \coloneq \eigen(\lvecsquare) \cap \eigen(\tilde{L}_i) \cap \eigen(\hrel).
    \]
    But since the spectrum of \(\lvecsquare\) and \(\tilde{L}_i\) is entirely pure point, \(\eigen(\lvecsquare) = \eigen(\tilde{L}_i) = \ltworthree\). Thus, \(\bhilbert = \eigen(\hrel)\). In order to find the elements of \(\bhilbert\), we consider the following equations.
    \begin{align*}
        \hrel \flme &= E \flme,\\
