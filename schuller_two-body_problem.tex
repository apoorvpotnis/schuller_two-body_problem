...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
	...
	\end{align*}
	If $\tilde{\psi} \in \schwartz(A) \rightarrow \schwartz(A)$ corresponds to $\psi$ in spherical co-ordinates\footnote{\,Prof.\@ Schuller uses $F$ to denote $\tilde{\psi}$. Note that even though $T$ is only surjective and not injective, $T^{-1}(\psi)$ is a measure zero set for any $\psi \in \schwartzrthree$. Thus, we don't need to make any further identifications in $\schwartz(A)$.}, we have
	\begin{align*}
		\tilde{\psi} = \psi \cdot T
	\end{align*}
	...
	\end{align*}
	where $\tilde{L}_3 \colon \schwartz(A)$ denotes the angular momentum operator in spherical co-ordinates.
	We now restrict the domain of $T$ suitably\footnote{\,More precisely, we take the domain to be $\tilde{A} \coloneq (0, +\infty) \times (0,\pi) \times (0,2\pi)$. This does not cause any problems as we are excluding only measure zero sets which are irrelevant when considering wave-functions.} such that it becomes injective and $T^{-1}$ becomes a well defined function. Thus, applying chain rule to $\psi = \tilde{\psi}T^{-1}$ gives us
	\begin{align*}
	...
	\end{align*}
	Substituting the above in $L_1 = -\i\hbar(y\partial_z - z\partial_y)$, we get
	\begin{align*}
