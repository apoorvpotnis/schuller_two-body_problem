...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
	...
	Consider a Lie algebra over \(\cc\) with the basis \(\{J_1, J_2, J_3\}\), where \(J_i\)'s are linear operators on a Hilbert space and the Lie bracket is given by the commutator. Furthermore, the following conditions are satisfied.
	\begin{enumerate}
	...
	We need to show that each \(J_i\) has a pure point spectrum.
