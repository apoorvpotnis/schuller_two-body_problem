...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
    ...
    \]
    Each \(\ltworthree_n\) is invariant under the action of any unitary one-parameter group \(U_k\). As each \(\ltworthree_n\) is finite-dimensional, every vector in \(\psi \in \ltworthree_n\) is smooth for \(U_k\), i.e. the mapping
    \begin{align*}
        s &\colon \rr \rightarrow \ltworthree_n,\\
        s &\colon t \mapsto (U_k(t))\psi,
    \end{align*}
    is differentiable in the norm topology on \(\ltworthree_n\). Thus, each \(\ltworthree_n \subset \domain_{J_k}\).\footnote{Note that even though each subset belongs to the domain, the union of all the subsets does not. The direct sum in the decomposition of \(\ltworthree\) is a Hilbert direct sum, and not an algebraic direct sum.}
