...
% 			\node[blue] at (1.4,-1.5) {$-1/r$};
    ...
    \end{thm}
    This result has been asked as problem~4.5 in the book of Teschl~\cite[p.~138]{Teschl}. This justifies the assumption of considering simultaneous eigenvectors of \(\symbfit{L}^2 \coloneq L_1^2 + L_2^2 + L_3^2\) and \(L_3\) in the usual procedure using ladder operators.
    ...
    Depending on \(H_\text{rel}\), a common orthonormal eigenbasis of \(\symbfit{L}\kern-1pt^2\), \(\tilde{L}_i\), and \(\hrel\) can possibly exist, as all three operators commute with each other pairwise. We for the moment \textit{assume} that this holds.\footnote{Clarify this later.} Thus, we have the following equations.
    \begin{align*}
        \hrel F^m_{l, E} &= E F^m_{l, E},\\
        \lvecsquare F^m_{l, E} &= l(l+1) F^m_{l, E},\\
        \lthree F^m_{l, E} &= m F^m_{l, E}.
    \end{align*}
    Here, \(F^m_{l,E}\) is the simultaneous eigenvector of \(\hrel\), \(\lvecsquare\) and \(\lthree\), indexed by \(l\), \(m\) and \(E\). \(E\) is the energy of \(F^m_{l,E}\).
